...
void Renderer::draw_filled_rect(const RenderListPtr &render_list, const Vec4 &rect, D3DCOLOR color)
{
	Vertex v[]
	{
		{ rect.x,          rect.y,          color },
...
void Renderer::draw_filled_rect(const Vec4 &rect, D3DCOLOR color)
{
	draw_filled_rect(render_list, rect, color);
}

void Renderer::draw_rect(const RenderListPtr &render_list, const Vec4 &rect, float stroke_width, D3DCOLOR color)
{
...
void Renderer::draw_rect(const Vec4 &rect, float stroke_width, D3DCOLOR color)
{
	draw_rect(render_list, rect, stroke_width, color);
}

void Renderer::draw_outlined_rect(const RenderListPtr &render_list, const Vec4 &rect, float stroke_width, D3DCOLOR outline_color, D3DCOLOR rect_color)
{
	draw_filled_rect(render_list, rect, rect_color);
	draw_rect(render_list, rect, stroke_width, outline_color);
}

void Renderer::draw_outlined_rect(const Vec4 &rect, float stroke_width, D3DCOLOR outline_color, D3DCOLOR rect_color)
{
	draw_filled_rect(render_list, rect, rect_color);
	draw_rect(render_list, rect, stroke_width, outline_color);
}

void Renderer::draw_line(const RenderListPtr &render_list, const Vec2 &from, const Vec2 &to, D3DCOLOR color)
{
	Vertex v[]
	{
		{ from.x, from.y, color },
...
void Renderer::draw_line(const Vec2 &from, const Vec2 &to, D3DCOLOR color)
{
	draw_line(render_list, from, to, color);
}

void Renderer::draw_radar(const RenderListPtr &render_list, const Vec2 &position, float size /* = 150.f */, float stroke_width /* = 1.f */, D3DCOLOR outline_color /* = 0UL */, D3DCOLOR rect_color /* = 0UL */)
{
...
void Renderer::draw_radar(const Vec2 &position, float size /* = 150.f */, float stroke_width /* = 1.f */, D3DCOLOR outline_color /* = 0UL */, D3DCOLOR rect_color /* = 0UL */)
{
	return draw_radar(render_list, position, size, stroke_width, outline_color, rect_color);
}

void Renderer::draw_circle(const RenderListPtr &render_list, const Vec2 &position, float radius, D3DCOLOR color /* = 0UL */)
{
...
void Renderer::draw_circle(const Vec2 &position, float radius, D3DCOLOR color /* = 0UL */)
{
	draw_circle(render_list, position, radius, color);
}

void Renderer::draw_pixel(const RenderListPtr &render_list, const Vec2 &position, D3DCOLOR color /* = 0UL */)
{
	draw_filled_rect(render_list, { position.x, position.y, 1.f, 1.f }, color);
}

void Renderer::draw_pixel(const Vec2 &position, D3DCOLOR color /* = 0UL */)
{
	draw_pixel(render_list, position, color);
}

void Renderer::draw_pixels(const RenderListPtr &render_list, const Vec2 &position, float square, D3DCOLOR color /* = 0UL */)
{
	draw_filled_rect(render_list, { position.x - 0.5f * square, position.y - 0.5f * square, square, square }, color);
}

void Renderer::draw_pixels(const Vec2 &position, float square, D3DCOLOR color /* = 0UL */)
{
...
void Renderer::draw_text(const RenderListPtr &render_list, FontHandle font, Vec2 position, const std::string &text, D3DCOLOR color, std::uint8_t flags)
{
	if (font.id < 0 || font.id >= std::size(fonts))
		throw std::exception(std::string("Renderer::draw_text: Bad font handle (identifier: {})!").c_str());

	fonts[font.id]->draw_text(render_list, { position.x, position.y }, text.c_str(), color, flags);
}

void Renderer::draw_text(FontHandle font, Vec2 position, const std::string &text, D3DCOLOR color, std::uint8_t flags)
{
...
Vertex::Vertex(Vec4 position, D3DCOLOR color) : position(position), color(color)
{
}

Vertex::Vertex(Vec4 position, D3DCOLOR color, Vec2 tex) : position(position), color(color), tex(tex)
{
}

Vertex::Vertex(Vec3 position, D3DCOLOR color) : position(position, 1.f), color(color)
{
}

Vertex::Vertex(Vec2 position, D3DCOLOR color) : position(position.x, position.y, 1.f, 1.f), color(color)
{
}

Vertex::Vertex(float x, float y, float z, D3DCOLOR color) : position(x, y, z, 1.f), color(color)
{
}

Vertex::Vertex(float x, float y, D3DCOLOR color) : position(x, y, 1.f, 1.f), color(color)
{
}
...
void throw_if_failed(HRESULT hr)
{
	if (FAILED(hr))
	{
		throw std::exception(std::string("Crucial Direct3D 9 operation failed! Code: %X").c_str());
	}
}